...
def require_payload_role(payload, *allowed):
    role = str(payload.get("role") or "").upper()
    if role not in {item.upper() for item in allowed}:
        raise HTTPException(status_code=403, detail="This portal is not available for your role")
...
def me(payload=Depends(get_current_user), db:Session=Depends(get_db)):
    ...
    return {"UserID":user.UserID,"FullName":user.FullName,"Email":user.Email,"Phone":user.Phone,"Role":user.role.RoleName if user.role else None,"Permissions":get_user_permission_codes(db,user)}
...
def student_dashboard(payload=Depends(get_current_user), db:Session=Depends(get_db)):
    require_payload_role(payload, "STUDENT")
...
def client_dashboard(payload=Depends(get_current_user), db:Session=Depends(get_db)):
    require_payload_role(payload, "CLIENT")
