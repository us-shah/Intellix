...
export const lmsApi = {
  ...
  deleteCourse: async (id:number) => (await api.delete(`/lms/courses/${id}`)).data,
  ...
  updateLesson: async (lessonId:number, body:Partial<Lesson>) => (await api.put<Lesson>(`/lms/lessons/${lessonId}`, body)).data,
  deleteLesson: async (lessonId:number) => (await api.delete(`/lms/lessons/${lessonId}`)).data,
  courseProgress: async (courseId:number) => (await api.get<{progress:number;completed_lesson_ids:number[]}>(`/lms/courses/${courseId}/progress`)).data,
  completeLesson: async (lessonId:number) => (await api.post<{progress:number;completed_lesson_id:number}>(`/lms/lessons/${lessonId}/complete`)).data,
